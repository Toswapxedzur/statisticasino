<script>
  ...
  function nav(patch) {
    const p = new URLSearchParams({ metric: data.metric, tf: data.timeframe, scope: data.scope, ...patch });
    goto(`/leaderboards?${p}`, { keepFocus: true, noScroll: true });
  }
  const fmt = (n) => Number(n).toLocaleString();
  const isNet = $derived(data.metric === "net");
  const unit = $derived(isNet ? "net at tables" : "chips");
</script>

<svelte:head><title>Leaderboards — Riverside</title></svelte:head>
...
<div class="wrap">
  ...
  <div class="controls">
    <div class="seg">
      <button class="sb" class:on={data.metric === "chips"} onclick={() => nav({ metric: "chips" })}>Chips</button>
      <button class="sb" class:on={data.metric === "net"} onclick={() => nav({ metric: "net" })}>Net winnings</button>
    </div>
    {#if isNet}
      <div class="seg">
        {#each [["all", "All-time"], ["week", "This week"], ["month", "This month"]] as [v, l]}
          <button class="sb" class:on={data.timeframe === v} onclick={() => nav({ tf: v })}>{l}</button>
        {/each}
      </div>
    {/if}
    <div class="seg">
      <button class="sb" class:on={data.scope === "global"} onclick={() => nav({ scope: "global" })}>Global</button>
      <button class="sb" class:on={data.scope === "friends"} onclick={() => nav({ scope: "friends" })} disabled={!data.signedIn}>Friends</button>
    </div>
  </div>

  {#if data.rows.length === 0}
    <div class="empty card"><p class="muted">{data.scope === "friends" ? "No friends on this board yet — add some friends!" : "No ranked players yet."}</p></div>
  {:else}
    <div class="board">
